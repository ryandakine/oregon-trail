...
`,
  natural: `
          ...
          /________||________\\
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
       carved names of those who passed
`,
  river_crossing: `
     ════════════════════════════════════
                  FAR BANK
...
`,
  settlement: `
     ...
     | |   | |  | ||  || | |  SUPPLIES  |
     |_|___|_|__|_||__||_|_|____________|
     ─────────────────────────────────────
      ~ wagons ~    ~ horses ~    ~ camp ~
`,
  destination: `
     ...
     ═══════════════════════════════
       The Willamette Valley awaits
`,
};

// ── Event Art (header illustrations for event screens) ──
...
const STORE_ART = `
 ...
 ╠═══════════════════════════════════════════════════╣
 ║  ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌───┐ ║
 ║  │FLOUR│ │AMMO │ │CLOTH│ │SPARE│ │MEDS │ │OXN│ ║
 ║  │ ░░░ │ │ ░░░ │ │ ░░░ │ │ ░░░ │ │ ░░░ │ │░░░│ ║
 ║  │ ░░░ │ │ ░░░ │ │ ░░░ │ │ ░░░ │ │ ░░░ │ │░░░│ ║
 ║  └─────┘ └─────┘ └─────┘ └─────┘ └─────┘ └───┘ ║
 ╚═══════════════════════════════════════════════════╝`;
...
const WAGON_TRAVEL = `
        ...
        |   / .'{_}  {_}'.  \\   |
        |  |  |___________|  |  |
     ___|__|__________________|__|___
    |   :::::::::::::::::::::::::::  |
...
╚═══════════════════════════════════════════════════════════╝
 ...
 ║ / ' . ' . ' |   / .'{_}  {_}'.  \\   |  / . ' . ' \\ ║
 ║ ~~~~~~~~~~~~|  |  |___________|  |  |~~~~~~~~~~~~~~ ║
 ║ ________|___|__|__________________|__|___________   ║
 ║         |   :::::::::::::::::::::::::::  |          ║
 ║         |________________________________|          ║
 ║            (@)          |          (@)              ║
 ║═════════════════════════════════════════════════════║
 ║    Independence, Missouri ─── Spring, 1848          ║
 ╚═══════════════════════════════════════════════════════╝
`.trim();
  ...
  renderStore(data) {
    ...
    const render = () => {
      ...
      this.$narrative.innerHTML = `
        <div class="terrain-art"><pre>${STORE_ART}</pre></div>
        <div class="narrative-block">
  ...
  // EVENT
  _getEventArtType(title, description) {
    const text = ((title || '') + ' ' + (description || '')).toLowerCase();
    if (/storm|rain|snow|lightning|thunder|hail|blizzard|dust/.test(text)) return 'weather';
    if (/trade|trader|merchant|buy|sell|barter/.test(text)) return 'trade';
    if (/cholera|dysentery|fever|sick|ill|disease|plague|measles|typhoid/.test(text)) return 'disease';
    if (/danger|wolf|snake|fire|stampede|attack|bandit|thief|rob/.test(text)) return 'danger';
    if (/found|discover|spring|creek|water|berry|herb/.test(text)) return 'discovery';
    if (/stranger|traveler|family|camp|wagon train|indian|native/.test(text)) return 'encounter';
    if (/dead|death|grave|buried|died|kill/.test(text)) return 'death';
    return 'default';
  }
  ...
  async renderEvent(eventData) {
    ...
    // Event art header
    const artType = this._getEventArtType(eventData.title, eventData.description);
    const art = EVENT_ART[artType] || EVENT_ART.default;
